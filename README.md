...
- **搜索功能**: 快速搜索笔记标题和内容
- **响应式设计**: 支持桌面和移动设备
- **智能权限控制**: 未登录时点击新建笔记会根据环境变量配置状态显示相应提示
- 支持回滚：可以快速回滚到之前的版本
...
当用户未登录时点击新建笔记按钮，系统会根据环境变量配置状态显示不同的提示：

### 环境变量未配置
如果 `VITE_REPO_OWNER` 或 `VITE_REPO_NAME` 未配置，会显示：
> "在创建笔记之前，您需要先配置环境变量。请在配置后前往设置页面查看是否生效。"

### 环境变量已配置
如果环境变量已配置但 `VITE_ADMIN_PASSWORD` 已设置，会显示管理员密码输入框：
> "您需要输入管理员密码才能创建笔记。"

### 已登录状态
如果用户已通过管理员密码验证或GitHub OAuth登录，直接跳转到笔记编辑页面。
