...
> **重要提示**: 在部署到 Cloudflare Pages 之前，请确保已在 Cloudflare Pages 控制台中正确配置了所有必需的环境变量。详细配置步骤请参考 [DEPLOYMENT.md](DEPLOYMENT.md) 文件。
...
3. **配置环境变量**：
   - 在项目设置中添加以下环境变量：
     - `VITE_R2_ACCOUNT_ID`: 你的 R2 账户 ID
     - `VITE_R2_ACCESS_KEY_ID`: 你的 R2 访问密钥 ID
     - `VITE_R2_SECRET_ACCESS_KEY`: 你的 R2 秘密访问密钥
     - `VITE_R2_BUCKET_NAME`: 你的 R2 存储桶名称
4. **部署**: 每次推送到 `main` 分支时自动触发部署
...
- [ ] 网站正常访问
- [ ] 笔记内容正确加载
- [ ] 静态 JSON 文件可访问（如 `/public-notes.json`）
- [ ] 无 CORS 错误
1. **Fork 项目**
...
1. **检查环境变量配置**：
   - 确认在 Cloudflare Pages 控制台中已设置所有必需的环境变量
   - 运行 `npm run debug-build` 检查环境变量状态
   - 运行 `npm run test-cloudflare-env` 测试 Cloudflare Pages 环境

2. **检查构建日志**：
   - 在 Cloudflare Pages 控制台中查看构建日志
   - 确认 `npm run build:pages` 命令执行成功
   - 检查是否有环境变量缺失的错误信息

3. **验证静态内容生成**：
   - 确认 `dist` 目录中包含 `public-notes.json`、`all-notes.json` 和 `build-info.json` 文件
   - 访问这些 JSON 文件，确认返回的是 JSON 数据而不是 HTML
...
- **[部署指南](DEPLOYMENT.md)** - 详细的 Cloudflare Pages 部署说明和故障排除
